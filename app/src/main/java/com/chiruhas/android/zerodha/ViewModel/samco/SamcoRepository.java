...
public class SamcoRepository {

    Retrofit retrofit;
    SamcoClient samcoClient;
    ...
    public SamcoRepository() {
        retrofit = new Retrofit.Builder().baseUrl("http://ec2-13-235-73-156.ap-south-1.compute.amazonaws.com:5000/samco/").addConverterFactory(GsonConverterFactory.create()).build();

        samcoClient = retrofit.create(SamcoClient.class);
    ...
    public LiveData<List<GodModel>> getEquity() {
        Call<List<GodModel>> call = samcoClient.getEquity();
    ...
    public LiveData<List<Commodity>> getCommodity() {
        Call<List<Commodity>> call = samcoClient.getCommodity();
    ...
    public LiveData<List<Futures>> getFutures() {
        Call<List<Futures>> call = samcoClient.getFutures();
    ...
    public LiveData<List<Currency>> getCurrency() {
        Call<List<Currency>> call = samcoClient.getCurrency();
